#include-once
#include <_WinAPI_GetSystemInfo.au3>	; _WinAPI_GetSystemInfo(6) -> CPU count
; ===============================================================================================================================
...
;	version 2010.07.01
;
; AutoIT version requirements: Minimum: AutoIT v3.3.6.0 (3.2.12.1 Unicode if alter DLL* types)
;	OS: Windows 2000+
;
; Functions:
...
;	_PDH_DebugWrite()				; Will write output to Console if $PDH_DEBUGLOG=True
...
;
; Dependencies:
;	<_WinAPI_GetSystemInfo.au3>		; System basic info, used for CPU count info
;
; See also:
...
;
; NOTES:
;	Performance Counters Registry Key Values which affect these functions,
;		if _PDH_Init() doesn't have write access to HKLM branch and one/both are set to 1:
;
;	  Key: HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\PerfOS\Performance
;	  Value: Disable Performance Counters
;
;	Another (not sure which is necessary for which Counters):
;	  Key: HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\PerfProc\Performance
;	  Value: Disable Performance Counters
;
;	Functions would probably return 'PDH_CSTATUS_NO_OBJECT' return if these Values is/are set to 1,
;		and _PDH_Init() wasn't able to change them.
;
; Additional:
...
;		Wildcard Counters may hep, but there are modules bundled now that handle Process Counters effectively.
;
...
; Used by Init/UnInit if certain Registry values, 'Disable Performance Counters', are turned on
Global Const $_PDH_PerfOSRegStr="HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\PerfOS\Performance"
Global Const $_PDH_PerfProcRegStr="HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\PerfProc\Performance"
Global Const $_PDH_PerfRegValue="Disable Performance Counters"
Global $_PDH_aRegBackup[2]
...
; Used to toggle Console-Output on or off
Global $PDH_DEBUGLOG=True
...
;
; Simple function to replace 'ConsoleWrite' that allows Debug output to be silenced (see $PDH_DEBUGLOG)
...
Func _PDH_DebugWrite($sString,$bAddCRLF=True)
	If Not $PDH_DEBUGLOG Then Return
	ConsoleWrite($sString)
	If $bAddCRLF Then ConsoleWrite(@CRLF)
EndFunc
...
Func __PDH_LocalizeCounter($sNonLocalizedCounter)
	...
	; Machine name?
	If $aSplitPath[0]>3 And $aSplitPath[4]<>"" Then $sMachineName='\\'&$aSplitPath[4]
	; Assemble localized counter name
	Return $sMachineName&'\'&_PDH_GetCounterNameByIndex(Number($aSplitPath[1]),$sMachineName)&$aSplitPath[3]& _
		'\'&_PDH_GetCounterNameByIndex(Number($aSplitPath[2]),$sMachineName)
EndFunc
...
;		@error = -1 = Global Variables have been mucked about with!
;		@error = 16 = Could not initialize (due to Registry settings)
;
; Author: Ascend4nt
; ===============================================================================================================================
...
Func _PDH_Init()
	...
	; This shouldn't happen. If it does - someone screwed up! Return -1 error code to flag this
	If $_PDH_hDLLHandle<>-1 Then
		$_PDH_bInit=False
		Return SetError(-1,0,False)
	EndIf

	; Check important 'Disable Performance Counter' Values
	$_PDH_aRegBackup[0]=RegRead($_PDH_PerfOSRegStr,$_PDH_PerfRegValue)
	; Is the key set to a non-zero number (i.e. 'Disable Performance Counter' = TRUE)?
	If IsNumber($_PDH_aRegBackup[0]) And $_PDH_aRegBackup[0] Then
		; Set the value to 0 (False)
		RegWrite($_PDH_PerfOSRegStr,$_PDH_PerfRegValue,"REG_DWORD",0)
	EndIf

	$_PDH_aRegBackup[1]=RegRead($_PDH_PerfProcRegStr,$_PDH_PerfRegValue)
	; Is the key set to a non-zero number (i.e. 'Disable Performance Counter' = TRUE)?
	If IsNumber($_PDH_aRegBackup[1]) And $_PDH_aRegBackup[1] Then
		; Set the value to 0 (False)
		RegWrite($_PDH_PerfProcRegStr,$_PDH_PerfRegValue,"REG_DWORD",0)
	EndIf

	; Now grab a handle to the PDH.DLL
	...
	If $_PDH_hDLLHandle=-1 Then
		...
		If IsNumber($_PDH_aRegBackup[0]) And $_PDH_aRegBackup[0] Then _
			RegWrite($_PDH_PerfOSRegStr,$_PDH_PerfRegValue,"REG_DWORD",$_PDH_aRegBackup[0])
		If IsNumber($_PDH_aRegBackup[1]) And $_PDH_aRegBackup[1] Then _
			RegWrite($_PDH_PerfProcRegStr,$_PDH_PerfRegValue,"REG_DWORD",$_PDH_aRegBackup[1])
	...
	If @error Then
		...
		If $_PDH_iCPUCount="" Then
			$_PDH_iCPUCount=1
		Else
			$_PDH_iCPUCount=Number($_PDH_iCPUCount)
		EndIf
	EndIf
...
Func _PDH_UnInit($hPDHQueryHandle=-1)
	; If not initialized, nothing to Uninitialize
	If Not $_PDH_bInit Then Return True
	; If DLL handle is -1, then nothing to Uninitialize
	;	(technically, $_PDH_hDLLHandle should be -1, but one extra check couldn't hurt..)
	If $_PDH_hDLLHandle=-1 Then
		$_PDH_bInit=False
		Return True
	EndIf
	; Free Query Handle (if valid). We don't care if there was errors, since we will be closing DLL
	If IsPtr($hPDHQueryHandle) Then _PDH_FreeQueryHandle($hPDHQueryHandle)
...
Func _PDH_ValidatePath($sCounterPath)
	...
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then
		$hPDHDLL="pdh.dll"
	Else
	...
	Local $aRet=DllCall($_PDH_hDLLHandle,"long","PdhValidatePathW","wstr",$sCounterPath)

	; DLL call error?
	If @error Then
		; Only needed to allow _PDH_DebugWrite() msg (which resets @error)
		$tErr=@error
		_PDH_DebugWrite("Path '"&$sCounterPath&"' caused a DLL-Call error")
	...
	If $aRet[0] Then
		...
		_PDH_DebugWrite("Path '"&$sCounterPath&"', per PdhValidatePathW, is invalid! [Return code: "&Hex($_PDH_iLastError)&']')
...
; ===============================================================================================================================
; Func _PDH_BrowseCounters($sTitle="",$hWnd=0)
;
...
;
; Returns:
;	Success: String containing Counter Selection (or wildcard selection), @error=0
...
Func _PDH_BrowseCounters($sTitle="",$hWnd=0)
	Local $hPDHDLL,$aRet,$iBCFlags,$hPDHBCCallback,$sReturnStr=""
...
#cs
	...
	; 2 = Allow only a single counter selection/add
	...
	; 128 = Disable Machine Selection - If set, user cannot access local machine counters
...
#ce
#cs
	; For now, we only allow one selection (or a wildcard selection).
	;  In the future, may allow more - but need to increase buffer (unknown count, so can't do 2048*max number)
	;	Will also need to use __PDH_StringGetNullTermMemStr() in a loop to retrieve them all! (MultiSz string)
#ce
	; Note: on Vista (and maybe Win7+), setting flag 2 will cause a blank-string return (with 'success' result) for many counters
	$iBCFlags=1+4+16

	; If $bLocalPCOnly Then $iBCFlags+=8	; in the future, allow option to choose local PC Counters only?

	; Create the return buffer, Set it to PDH_MAX_COUNTER_PATH size(2048).
	;	This will need to be modified if multiple selections are allowed to "byte[xxxxx]", and strings retrieved in loop
	$stCounterSelectBuffer=DllStructCreate("wchar[2048]")
	...
	DllStructSetData($stBrowseDlgConfig,1,$iBCFlags)	; Flags
 	DllStructSetData($stBrowseDlgConfig,2,$hWnd)	; HWndOwner (NULL is fine, unless want to use parent GUI)
;~ 	DllStructSetData($stBrowseDlgConfig,3,Ptr(0))	; DataSource (NULL = don't use log file)
...
;~ 	DllStructSetData($stBrowseDlgConfig,6,Ptr(0))	; Callback function (NULL is fine)
...
#cs
	; ----------------------------------------------------
	;  -  Detail Levels  -
	; PERF_DETAIL_NOVICE		0x00000064 (decimal 100)
	;	For the novice or more advanced user.
	; PERF_DETAIL_ADVANCED	0x000000C8 (decimal 200)
	;	For the advanced user.
	; PERF_DETAIL_EXPERT		0x0000012C (decimal 300)
	;	For the expert user.
	; PERF_DETAIL_WIZARD		0x00000190 (decimal 400)
	;	For the system designer.
	; ----------------------------------------------------
#ce
	DllStructSetData($stBrowseDlgConfig,9,0x190)	; Default Detail Level (0x190 = PERF_DETAIL_WIZARD)

	; Did user specify a caption?
	If $sTitle<>"" Then
...
;~ 	Else
		; Default caption (pass NULL)	[zeroed at structure creation]
;~ 		DllStructSetData($stBrowseDlgConfig,10,Ptr(0))	; Dialog Box Caption (NULL = default "Browse Performance Counters")
	EndIf

	; Make the call
	$aRet=DllCall($hPDHDLL,"long","PdhBrowseCountersW","ptr",DllStructGetPtr($stBrowseDlgConfig))
	If @error Then Return SetError(2,@error,$sReturnStr)

	If $aRet[0] Then
		; PDH_DIALOG_CANCELLED (0x800007D9)? - Then no selection was made
		If $aRet[0]=0x800007D9 Then
			_PDH_DebugWrite("_PDH_Browse_Counters Dialog Cancelled, returning empty string")
			Return $sReturnStr
		EndIf
		; Otherwise unknown error
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("PdhBrowseCountersW non-zero error code:" & Hex($_PDH_iLastError))
		Return SetError(3,$_PDH_iLastError,$sReturnStr)
	EndIf
	; Grab the selection string
	$sReturnStr=DllStructGetData($stCounterSelectBuffer,1)
	; DEBUG
	_PDH_DebugWrite("Selected Counter from _PDH_Browse_Counters:" & $sReturnStr)
	Return $sReturnStr
EndFunc
...
Func _PDH_GetCounterList($sCounterWildcardPath,$bReturnAsString=False)
	...
	If $aRet[0]<>0x800007D2 Then
		$_PDH_iLastError=$aRet[0]
		; DEBUG
		_PDH_DebugWrite("PdhExpandWildCardPathW 1st call unsuccessful, return:" & Hex($_PDH_iLastError))
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		; DEBUG
		_PDH_DebugWrite("PdhExpandWildCardPathW 2nd call unsuccessful, return:" & Hex($_PDH_iLastError))
		...
		If $aRet[0]=0x800007D2 Then
			_PDH_DebugWrite("PdhExpandWildCardPathW 2nd call returned 'PDH_MORE_DATA'. 1st reported Bufsize:" & $iBufSize & _
	...
	Else
		...
		Else
			...
			$aCounterList[0]=0
			; DEBUG
			_PDH_DebugWrite("PdhExpandWildCardPathW data invalid")
...
;	@error = -1 = not *properly* initialized ($_PDH_hDLLHandle is set, but $_PDH_bInit=False)
;	@error = 2 = DLL Call error, @extended=DLLCall error (see AutoIt Help)
...
Func _PDH_GetNewQueryHandle($iUserInfo=0)
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("_PDH_GetNewQueryHandle call returned with PDH error:" & Hex($_PDH_iLastError))
...
Func _PDH_FreeQueryHandle(ByRef $hPDHQueryHandle)
	...
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,False)
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("PdhCloseQuery DLL call unsuccessful for handle: "&Hex($hPDHQueryHandle)&", return:" & Hex($_PDH_iLastError))
...
Func _PDH_AddCounter($hPDHQueryHandle,$sCounterPath,$iUserInfo=0)
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,0)
...
;~ 	_PDH_DebugWrite("_PDH_AddCounter called w/ Handle (" &$hPDHQueryHandle&") Adding: '" &$sCounterPath & "' w/ param to associate:" & $iParam)
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("PdhAddCounterW error [path:'"&$sCounterPath&"'], return:" & Hex($_PDH_iLastError))
...
Func _PDH_AddCountersByArray($hPDHQueryHandle,Const ByRef $aCounterArray,$iStart=0,$iEnd=-1,$iBottomRows=0,$iColumns=0)
	...
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,$aCountersReturn)
...
Func _PDH_AddCountersByMultiStr($hPDHQueryHandle,Const ByRef $sMultiStr,$sSepChar,$iBottomRows=0,$iColumns=0)
	...
	Else
		...
		$aCounterList[0][0]=0
		; DEBUG
		_PDH_DebugWrite("_PDH_AddCountersByMultiStr() StringSplit() resulted in no data (invalid param)")
		Return SetError(1,0,$aCounterList)
	EndIf
EndFunc
...
Func _PDH_AddCountersByWildcardPath($hPDHQueryHandle,$sCounterWildcardPath,$iBottomRows=0,$iColumns=0)
	...
	If @error Or $aCounterList[0]=0 Then
		...
		; DEBUG
		_PDH_DebugWrite("_PDH_AddCountersByWildcardPath call to PDH_GetCounterList failed")
...
Func _PDH_RemoveCounter(ByRef $hPDHCounterHandle)
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("PdhRemoveCounter error, return:" & Hex($_PDH_iLastError))
		Return SetError(3,$_PDH_iLastError,False)
	EndIf
	; DEBUG
...
; ===============================================================================================================================
; Func _PDH_GetCounterNameByIndex($iIndex,$sMachineName="")
;
...
; $sMachineName = *optional* parameter. If included, the machine name is extracted from the passed
...
Func _PDH_GetCounterNameByIndex($iIndex,$sMachineName="")
	...
	Else
		$hPDHDLL=$_PDH_hDLLHandle
	EndIf
	_PDH_DebugWrite("_PDH_GetCounterNameByIndex() call, Index:"&$iIndex&" [optional] Machine Name:"&$sMachineName&", PDH DLL 'handle' (or just 'pdh.dll'):" &$hPDHDLL)

	; Extract Machine Name (optional). All other parts of path (if included) are cleared
	$vMachineParam=StringRegExpReplace($sMachineName,"(\\\\[^\\]+)?(\\?.*)","$1")
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("_PDH_GetCounterNameByIndex non-zero error code:" & Hex($_PDH_iLastError))
...
; ===============================================================================================================================
; Func _PDH_GetCounterIndex($sCounterOrObject,$sMachineName="")
;
...
; $sMachineName = *optional* value -> the machine name, prefixed as normal with '\\' (Example: '\\PCNAME')
;
; Return:
...
Func _PDH_GetCounterIndex($sCounterOrObject,$sMachineName="")
	...
	Else
		$hPDHDLL=$_PDH_hDLLHandle
	EndIf
	_PDH_DebugWrite("_PDHGetCounterIndex() call, Counter or Object Name:'"&$sCounterOrObject&"', Machine Name (optional):'"&$sMachineName&"', PDH DLL 'handle' (or just 'pdh.dll'):" &$hPDHDLL)
	...
	$aRet=DllCall($hPDHDLL,"long","PdhLookupPerfIndexByNameW",$sMachineParamType,$sMachineName,"wstr",$sCounterOrObject,"dword*",0)
	If @error Then Return SetError(2,@error,-1)

	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("_PDHGetCounterIndex non-zero error code:" & Hex($_PDH_iLastError))
...
Func _PDH_GetCounterInfo($hPDHCounterHandle)
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,"")
	...
	If $aRet[0]<>0x800007D2 Or ($aRet[0]=0 And $aRet[3]=0) Then
		$_PDH_iLastError=$aRet[0]
		; DEBUG
		_PDH_DebugWrite("PdhGetCounterInfoW 1st call unsuccessful, return:" & Hex($_PDH_iLastError))
		Return SetError(3,$_PDH_iLastError,"")
	EndIf
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		; DEBUG
		_PDH_DebugWrite("PdhGetCounterInfoW 2nd call unsuccessful, return:" & Hex($_PDH_iLastError))
...
;
; Internal function - just updates the Counters attached to a Query Handle - doesn't actually
;	collect the values (that would be a separate DLL Call - PdhGetFormattedCounterValue)
;
...
Func _PDH_CollectQueryData($hPDHQueryHandle,$bSkipChecks=False)
	...
	If $aRet[0] Then
		$_PDH_iLastError=$aRet[0]
		_PDH_DebugWrite("PdhCollectQueryData error, return:" & Hex($_PDH_iLastError))
		Return SetError(3,$_PDH_iLastError,False)
	EndIf
	Return True
EndFunc
...
Func _PDH_UpdateCounter($hPDHQueryHandle,$hPDHCounterHandle,$sCounterPath="",$bGrabValueOnly=False)
	; Properly initialized?
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,-1)
	...
	Local $stCounterValue=DllStructCreate("long;int64")	; PDH_FMT_COUNTERVALUE
#cs
	;-----------------------------------------------------
	;  -  Format Options  -
...
#ce
	...
	If $aRet[0] Then
		...
		If $aRet[0]=0xC0000BC6 And DllStructGetData($stCounterValue,1)=0x800007D1 Then
			_PDH_DebugWrite("Invalid Data (invalid handle) message for Handle:" & $hPDHCounterHandle & _
				", Path [if passed]:" &$sCounterPath)
			Return SetError(-1,$_PDH_iLastError,-1)
		EndIf
		_PDH_DebugWrite("Error Calling PdhGetFormattedCounterValue for Handle:"&$hPDHCounterHandle& _
...
Func _PDH_UpdateWildcardCounter($hPDHQueryHandle,$hPDHCounterHandle,$sCounterWildcardPath="",$bGrabValueOnly=False)
	...
	; Properly initialized?
	If Not $_PDH_bInit Or $_PDH_hDLLHandle=-1 Then Return SetError(16,0,$aValueArray)
	; Check if Counter Handle is valid
	...
	If $aRet[0]<>0x800007D2 And Not ($aRet[0]=0 And $aRet[3] And @OSVersion="WIN_2000") Then
		$_PDH_iLastError=$aRet[0]
		; DEBUG
		_PDH_DebugWrite("PdhGetFormattedCounterArrayW 1st call unsuccessful, return:" & Hex($_PDH_iLastError)&", sz:"&$aRet[3])
...
Func _PDH_UpdateCounters($hPDHQueryHandle,ByRef $aPDHCountersArray,$iHandleIndex,$iCountIndex,$iStrIndex=-1,$iDeltaIndex=-1, _
	...
	$stCounterValue=DllStructCreate("long;int64")	; PDH_FMT_COUNTERVALUE
...
#ce
			...
			If @error Then
				; DLL Call error.. hmm..
				If $iStrIndex>=0 Then
					_PDH_DebugWrite("DLL Call error ("&@error&") at item("&$aPDHCountersArray[$i][$iStrIndex]&") (index #" &$i&")")
				Else
					_PDH_DebugWrite("DLL Call error ("&@error&") at index #" &$i&", Handle:"&$aPDHCountersArray[$i][$iHandleIndex])
				EndIf
			ElseIf $aRet[0] Then
				$_PDH_iLastError=$aRet[0]
				; PDH_INVALID_DATA? (0xC0000BC6). If CStatus also is PDH_CSTATUS_NO_INSTANCE (0x800007D1), counter no longer exists
				If $aRet[0]=0xC0000BC6 And DllStructGetData($stCounterValue,1)=0x800007D1 Then
					...
					If $iStrIndex>=0 Then
						_PDH_DebugWrite("Invalid Data (invalid handle) message for (" & $aPDHCountersArray[$i][$iStrIndex] & _
					...
					Else
						_PDH_DebugWrite("Invalid data message at index #" &$i&", Handle:"&$aPDHCountersArray[$i][$iHandleIndex])
					EndIf
					; Make a 'Remove Counter' call to remove it from PDH Query Handle
					_PDH_RemoveCounter($aPDHCountersArray[$i][$iHandleIndex])
					_PDH_DebugWrite("_PDH_RemoveCounter() call completed (for PDH error 'PDH_INVALID_DATA (0xC0000BC6)')")
				...
				Else
					; DEBUG
					If $iStrIndex>=0 Then
						_PDH_DebugWrite("Error Calling PdhGetFormattedCounterValue for ("&$aPDHCountersArray[$i][$iStrIndex]& _
							"), Return:" & Hex($_PDH_iLastError) & " CStatus:"&Hex(DllStructGetData($stCounterValue,1)))
					Else
						_PDH_DebugWrite("Error calling PdhGetFormattedCounterValue at index #" &$i& _
			...
			Else
				...
				If DllStructGetData($stCounterValue,1) Then _
					_PDH_DebugWrite("PdhGetFormattedCounterValue non-0 status for handle #" & $i & ":" & Hex(DllStructGetData($stCounterValue,1)))
